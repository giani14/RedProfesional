...
export default function ClientePerfil() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (user) {
        const { data: dataProfesional, error: errorProfesional } =
          await supabase
        ...
        setDataPro({
          ...
          contratos:
            dataProfesional?.filter((s) => s.estado === "aceptada").length || 0,
        });
      }
      if (user) {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.menuContainer}>
          ...
          <MenuOption
            ...
            onPress={() => router.push("/(profesional)/centroDeAyuda")}
          />
          <MenuOption
            icon="shield-checkmark-outline"
            label="Privacidad"
            onPress={() => router.push("/(profesional)/privacidad")}
          />
