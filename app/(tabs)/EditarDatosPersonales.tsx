...
export default function EditarDatosPersonales() {
  ...
  const formularioInvalido = errores.nombre || errores.correo || errores.telefono;
  ...
  const manejarGuardar = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Hubo un fallo al guardar los cambios. Intenta de nuevo.");
    } finally {
      setGuardando(false);
    }
  };
  ...
  if (cargando) {
    return <View style={styles.centrado}><ActivityIndicator size="large" color="#FFB100" /></View>;
  }
  ...
  return (
    <ScrollView style={styles.contenedor} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={hayCambios ? mostrarAlertaDescartar : () => router.back()}>
      ...
      <View style={styles.cuerpo}>
        ...
        <View style={styles.seccionFoto}>
          <View style={styles.contenedorAvatar}>
            ...
            <TouchableOpacity style={styles.botonCamara} onPress={seleccionarImagen}>
        ...
        <View style={styles.form}>
          ...
          <View style={[styles.inputContainer, errores.nombre && styles.inputErrorBorde]}>
            <Ionicons name="person-outline" size={20} color="#666" />
            <TextInput 
              style={styles.input}
              value={nombre}
              onChangeText={(txt) => {setNombre(txt); setHayCambios(true);}}
              placeholder="Juan Pérez García"
            />
          </View>
          {errores.nombre && <Text style={styles.errorTxt}>El nombre completo es obligatorio.</Text>}
          ...
          <View style={[styles.inputContainer, errores.telefono && styles.inputErrorBorde]}>
            <Ionicons name="call-outline" size={20} color="#666" />
            <TextInput 
              style={styles.input}
              value={telefono}
              keyboardType="phone-pad"
              onChangeText={(txt) => {setTelefono(txt); setHayCambios(true);}}
              placeholder="Ej: 77123456"
              maxLength={8}
            />
          </View>
          {errores.telefono && <Text style={styles.errorTxt}>Ingresa un número de Bolivia válido (8 dígitos).</Text>}
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput 
              style={styles.input}
              value={ubicacion}
              onChangeText={(txt) => {setUbicacion(txt); setHayCambios(true);}}
          ...
          <TouchableOpacity 
            style={[styles.btnGuardar, (formularioInvalido || guardando) && styles.btnDeshabilitado]}
...
const styles = StyleSheet.create({
  ...
  headerTitulo: { color: 'white', fontSize: 18, fontWeight: 'bold', marginLeft: 15 },
  cuerpo: { padding: 20 },
